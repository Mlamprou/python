...
counter1=1
max_distance=int(input("Please insert the maximum distance between landmarks that you want to walk (in km):"))
transport=[]
while counter1<len(visit):
    ...
    transport.insert(counter1-1,"walking")
    if distance[counter1-1][0]>max_distance:
            distance.insert(counter1-1, get_route_details("AIzaSyCcB6x1vFGflBctPmhwPaHWf2zdDghKVXY", visit[counter1-1], visit[counter1], "transit", when=None))
            transport.insert(counter1-1,"using public transit")
    counter1=counter1+1                
...
for l in visit:
          ...
          if counter2<len(distance):
           print('You will have to go there',transport[counter2],'for a distance of', distance[counter2][0], 'Km for', distance[counter2][1], 'minutes')
